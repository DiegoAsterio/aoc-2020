...
class TestMovements(unittest.TestCase):
    def test_moving_90_degrees_anticlockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('R', 90)
        res = np.array([[0], [-1]])
        self.assertTrue((r.turn(direction) == res).all())

    def test_moving_180_degrees_anticlockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('R', 180)
        res = np.array([[-1], [0]])
        self.assertTrue((r.turn(direction) == res).all())

    def test_moving_270_degrees_anticlockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('R', 270)
        res = np.array([[0], [1]])
        self.assertTrue((r.turn(direction) == res).all())

    def test_moving_90_degrees_clockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('L', 90)
        res = np.array([[0], [1]])
        self.assertTrue((r.turn(direction) == res).all())

    def test_moving_180_degrees_clockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('L', 180)
        res = np.array([[-1], [0]])
        self.assertTrue((r.turn(direction) == res).all())

    def test_moving_270_degrees_clockwise(self):
        direction = np.array([[1], [0]])
        r = Movement('L', 270)
        res = np.array([[0], [-1]])
        self.assertTrue((r.turn(direction) == res).all())
